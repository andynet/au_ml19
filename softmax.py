...
def softmax(X):
    ...
    logsum = np.log(np.sum(np.exp(X - max), axis=1, keepdims=True)) + max
    logsoftmax = X - logsum
    res = np.exp(logsoftmax)
    ### END CODE
    return res
...
class SoftmaxClassifier():
    ...
    def cost_grad(self, X, y, W):
        ...
        ### YOUR CODE HERE
        nll = -np.sum(Yk * np.log(softmax(X @ W)))    # NLL(X|W)
        cost = nll / X.shape[0]

        grad_nll = -(X.T @ (Yk - softmax(X @ W)))       # grad NLL(X|W)
        grad = grad_nll / X.shape[0]
        ### END CODE
        return cost, grad
